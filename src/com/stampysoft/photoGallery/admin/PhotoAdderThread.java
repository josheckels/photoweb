...
public class PhotoAdderThread extends Thread
{
    ...
    public void run()
    {
        try
        {
            ...
            SwingUtilities.invokeAndWait(new Runnable()
            {
                public void run()
                {
                    photoList.addAll(PhotoOperations.getPhotoOperations().getAllPhotos());
                }
            });
    ...
    {
        Resolution[] allResized = photo.getResizedDimensions();
        for (Resolution resized : allResized)
        {
            File resizedFile = new File(PhotoOperations.getPhotoOperations().toURI(resized.getURI()));
    ...
    private Photo addNewPhoto(File photoFile) throws URISyntaxException, PhotoManipulationException, InterruptedException, InvocationTargetException
    {
        ...
        SwingUtilities.invokeAndWait(new Runnable()
        {
            public void run()
            {
                returnValue[0] = PhotoOperations.getPhotoOperations().savePhoto(photoToSave);
            }
        });
